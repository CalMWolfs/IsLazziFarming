...
import com.calmwolfs.islazzifarming.utils.ClipboardUtils
import com.calmwolfs.islazzifarming.utils.KeyboardUtils
...
object CopyErrorCommand {
    ...
    fun command(array: Array<String>) {
        if (array.size != 1) {
            ChatUtils.chat("§cUse /lzcopyerror <error id> or just click on the error in chat!")
            return
        }
        ...
        ChatUtils.chat(errorMessage?.let {
            ...
            "§e[IsLazziFarming] $name copied into the clipboard, please report it on the GitHub!"
        } ?: "§c[IsLazziFarming] Error id not found!")
    }
    ...
    fun logError(throwable: Throwable, message: String) {
        ...
        errorMessages[randomId] = "```\nIsLazziFarming Mod ${IsLazziFarmingMod.version}: $rawMessage\n \n$stackTrace\n```"
        fullErrorMessages[randomId] =
            "```\nIsLazziFarming Mod ${IsLazziFarmingMod.version}: $rawMessage\n(full stack trace)\n \n$fullStackTrace\n```"

        ChatUtils.clickableChat(
            "§c[IsLazziFarming ${IsLazziFarmingMod.version}]: $message§c. Click here to copy the error into the clipboard.",
            "lzcopyerror $randomId"
        )
    }
}
...
private fun Throwable.getExactStackTrace(full: Boolean, parent: List<String> = emptyList()): List<String> = buildList {
    ...
    val replace = mapOf(
        "com.calmwolfs.islazzifarming" to "LZ",
    )
